...
def player_setup_screen():
    ...
    add_button = Button(SCREEN_WIDTH//2 - 100, SCREEN_HEIGHT//2 + 60, 200, 40, "Add Player", GREEN, (0, 200, 0))
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if add_button.is_clicked(mouse_pos, event) and input_text.strip() and len(players) < 4:
            ...
            if event.type == KEYDOWN:
                if active:
                    if event.key == K_RETURN:
                        if input_text.strip() and len(players) < 4:
                            players.append(input_text.strip())
                            input_text = ""
        ...
        draw_text(screen, "Enter player names (2-4 players):", (SCREEN_WIDTH//2, 180), WHITE, font, True)
        
        # Draw input box
        pygame.draw.rect(screen, color, input_rect, 2, border_radius=5)
        draw_text(screen, input_text, (input_rect.x + 10, input_rect.y + 10))
        ...
        # Update buttons
        add_button.check_hover(mouse_pos)
        ...
        if len(players) >= 4:
            draw_text(screen, "Max players reached", (SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 50), WHITE, font, True)
